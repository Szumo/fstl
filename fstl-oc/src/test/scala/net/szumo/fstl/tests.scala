...
class Timings extends FlatSpec with SequentialNestedSuiteExecution {
  ...
  val letters = "alyuiopdfghjkzxcvbnm"
  ...
  "Matchers" should "have timings" in {
    val matcher = timeIt("FSTL: Creating a big matcher", StringMatcher(words, CaseInsensitive))
    timeIt("FSTL: Getting one match", matcher(string).next())
    //timeIt("Getting all matches", matcher(string).toVector)

  }
  "Regex" should "have timings" in {
    val pattern = timeIt("Regex: Creating a big regex", Pattern.compile(words.mkString("|"), Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE))
    timeIt("Regex: Getting one match", pattern.matcher(string).find())
    //timeIt("Getting all matches", pattern.matcher(string).start
  }
  ...
  "RE2" should "have timings" in {
    ...
    val regex = timeIt("RE2: Creating a big regex", new RE2(words.mkString("|"), options))
